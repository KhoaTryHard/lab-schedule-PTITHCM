...
LabSchedulePTIT.local.postman_environment.json.example
```
...
Có thể lưu log hoặc ghi chú kết quả ở ngoài repo khi cần đối chiếu trong quá trình kiểm thử.

## Ghi chú

Postman collection và environment trong thư mục này dùng để kiểm thử API khi chạy local.
