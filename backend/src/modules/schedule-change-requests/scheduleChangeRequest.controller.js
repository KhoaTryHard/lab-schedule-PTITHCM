...
async function getScheduleChangeRequest(req, res) {
  ...
  const item = await changeRequestService.getChangeRequestById(
    req.params.id,
    req.user,
  );
...
async function getScheduleChangeOptions(req, res) {
  const errors = validationResult(req);
  if (!errors.isEmpty()) {
    return fail(res, 400, "Validation failed", errors.array());
  }

  const result = await changeRequestService.getChangeRequestOptions(
    req.params.id,
    req.user,
  );
  ...
  return ok(
    res,
    result.options,
    "Successfully generated schedule change options",
  );
}
...
async function createScheduleChangeRequest(req, res) {
  ...
  return created(
    ...
    "Successfully created schedule change request",
  );
}
...
async function reviewScheduleChangeRequest(req, res) {
  ...
  return sendServiceResult(
    res,
    result,
    "Successfully reviewed schedule change request",
  );
}
