...
async function createSchedule(req, res) {
  ...
  const result = await createDraftSchedule(req.body, req.user);

  if (!result.created) {
    return fail(
      res,
      409,
      "Schedule constraints failed",
  ...
  return created(
    res,
    {
      ...
      schedules: result.schedules || (result.schedule ? [result.schedule] : []),
      created_count: result.created_count || 1,
...
async function listScheduleWeeks(req, res) {
  const weeks = await getScheduleWeeks({
    semester_id: req.query.semester_id,
    all: req.query.all,
  });

  return ok(res, weeks, "Successfully fetched schedule weeks");
}

async function listTimeSlots(req, res) {
  const timeSlots = await getActiveTimeSlots();
  return ok(res, timeSlots, "Successfully fetched time slots");
}
