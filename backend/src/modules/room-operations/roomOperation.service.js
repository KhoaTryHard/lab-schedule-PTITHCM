...
const { isValidRoomStatus } = require("../rooms/room.service");
...
const BLOCK_CREATE_STATUSES = new Set(["draft", "submitted"]);
const BLOCK_EDITABLE_STATUSES = new Set(["draft", "rejected"]);
const BLOCK_SUBMITTABLE_STATUSES = new Set(["draft", "rejected"]);
const BLOCK_CANCELLABLE_STATUSES = new Set(["draft", "submitted"]);

function isRoomBlockOwner(block, user) {
  return Number(block.requested_by_user_id) === Number(user?.id);
}

function canManageRoomBlock(user) {
  return [ROLES.ADMIN, ROLES.ACADEMIC_OFFICER].includes(user?.role_code);
}

function canMutateRoomBlock(block, user) {
  return canManageRoomBlock(user) || isRoomBlockOwner(block, user);
}
...
const ROOM_ISSUE_SELECT = `
  ...
  DATE_FORMAT(schedule.start_date, '%Y-%m-%d') AS schedule_start_date,
  DATE_FORMAT(schedule.end_date, '%Y-%m-%d') AS schedule_end_date,
...
const ROOM_BLOCK_SELECT = `
  ...
  DATE_FORMAT(block.start_date, '%Y-%m-%d') AS start_date,
  DATE_FORMAT(block.end_date, '%Y-%m-%d') AS end_date,
  block.block_status,
...
async function getRoomByInput(input, connection = pool) {
  ...
  if (!roomId && !roomCode) {
    return {
      ok: false,
      statusCode: 400,
      message: "room_id or room_code is required",
    };
  }
...
async function resolveDevice(input, roomId, connection = pool) {
  ...
  const deviceCode =
    normalizeText(input.device_code) ||
    (!deviceId ? normalizeText(rawDeviceId) : null);
  ...
  if (!device) {
    return {
      ok: false,
      statusCode: 400,
      message: "Device does not exist for the selected room",
    };
  }
  ...
  if (Number(device.room_id) !== Number(roomId)) {
    return {
      ok: false,
      statusCode: 400,
      message: "Device does not belong to the selected room",
    };
  }
...
async function createRoomIssue(input, user) {
  ...
  if (labScheduleEntryId) {
    ...
    if (
      ...
      return {
        ok: false,
        statusCode: 403,
        message: "Lecturers can only report issues for schedules they teach",
      };
    }
  }
  ...
  if (user.role_code === ROLES.LECTURER && !scheduleEntry) {
    return {
      ok: false,
      statusCode: 400,
      message: "Lecturer issue reports must include lab_schedule_entry_id",
    };
  }
  ...
  if (input.room_id && Number(input.room_id) !== Number(room.id)) {
    return {
      ok: false,
      statusCode: 400,
      message: "room_id does not match the selected schedule entry",
    };
  }

  if (
    input.room_code &&
    normalizeRoomCode(input.room_code) !== normalizeRoomCode(room.room_code)
  ) {
    return {
      ok: false,
      statusCode: 400,
      message: "room_code does not match the selected schedule entry",
    };
  }
  ...
  if (assignedToUserId && !(await existsById("users", assignedToUserId))) {
    return {
      ok: false,
      statusCode: 400,
      message: "assigned_to_user_id does not exist",
    };
  }

  const detectedAt = isValidDate(input.detected_at)
    ? new Date(input.detected_at)
    : new Date();
...
async function updateRoomIssue(id, input, user) {
  ...
  if (!current) {
    return {
      ok: false,
      statusCode: 404,
      message: "Room issue report not found",
    };
  }
  ...
  if (Object.prototype.hasOwnProperty.call(input, "issue_status")) {
    ...
    if (!ISSUE_STATUSES.has(nextStatus)) {
      return {
        ok: false,
        statusCode: 400,
        message: "issue_status is not supported",
      };
    }
...
function applyRoomBlockFilters(filters, user, conditions, params) {
  if (user?.role_code === ROLES.TECHNICIAN) {
    conditions.push("block.requested_by_user_id = ?");
    params.push(user.id);
  }
...
async function listRoomBlocks(filters, user) {
  const conditions = [];
  const params = [];

  applyRoomBlockFilters(filters, user, conditions, params);
...
async function createRoomBlock(input, user) {
  ...
  const requestedStatus =
    normalizeText(input.block_status || input.status) || "draft";

  if (!BLOCK_CREATE_STATUSES.has(requestedStatus)) {
    return {
      ok: false,
      statusCode: 400,
      message: "block_status must be draft or submitted",
    };
  }
  ...
  if (!BLOCK_TYPES.has(blockType)) {
    return {
      ok: false,
      statusCode: 400,
      message: "block_type is not supported",
    };
  }
  ...
  if (timeSlotId && !(await existsById("time_slots", timeSlotId))) {
    return {
      ok: false,
      statusCode: 400,
      message: "time_slot_id does not exist",
    };
  }
...
async function updateRoomBlock(id, input, user) {
  ...
  if (!current) {
    return {
      ok: false,
      statusCode: 404,
      message: "Room block request not found",
    };
  }

  if (!canMutateRoomBlock(current, user)) {
    return {
      ok: false,
      statusCode: 403,
      message: "You are not allowed to update this room block request",
    };
  }

  if (!BLOCK_EDITABLE_STATUSES.has(current.block_status)) {
    return {
      ok: false,
      statusCode: 409,
      message: "Only draft or rejected room block requests can be updated",
...
async function submitRoomBlock(id, input, user) {
  const current = await getRoomBlockRowById(id);
  ...
  if (!canMutateRoomBlock(current, user)) {
    return {
      ok: false,
      statusCode: 403,
      message: "You are not allowed to submit this room block request",
    };
  }

  if (!BLOCK_SUBMITTABLE_STATUSES.has(current.block_status)) {
    return {
      ok: false,
      statusCode: 409,
      message: "Only draft or rejected room block requests can be submitted",
      details: { current_status: current.block_status },
    };
  }
  ...
  await recordAuditLog({
    ...
    action_by_user_id: user.id,
    action_notes: {
      submit_notes: normalizeText(input.submit_notes),
    },
  });
...
async function cancelRoomBlock(id, input, user) {
  const current = await getRoomBlockRowById(id);
  ...
  if (!canMutateRoomBlock(current, user)) {
    return {
      ok: false,
      statusCode: 403,
      message: "You are not allowed to cancel this room block request",
    };
  }

  if (!BLOCK_CANCELLABLE_STATUSES.has(current.block_status)) {
    return {
      ok: false,
      statusCode: 409,
      message: "Only draft or submitted room block requests can be cancelled",
      details: { current_status: current.block_status },
    };
  }
  ...
  await pool.query(
    `UPDATE room_block_requests
     SET block_status = 'cancelled',
         ...
         updated_at = CURRENT_TIMESTAMP
     WHERE id = ?`,
    [normalizeText(input.review_notes), id],
  );
...
async function applyRoomBlockStatus(id, input, user) {
  const current = await getRoomBlockRowById(id);
  ...
  if (current.block_status !== "approved") {
    return {
      ok: false,
      statusCode: 409,
      message: "Only approved room block requests can update room status",
      details: { current_status: current.block_status },
    };
  }

  const nextRoomStatus = normalizeText(input.room_status) || "locked";

  if (!isValidRoomStatus(nextRoomStatus)) {
    return {
      ok: false,
      statusCode: 400,
      message: "room_status is not supported",
    };
  }
  ...
  const notes = (
    ...
    `Cập nhật trạng thái phòng từ yêu cầu khóa phòng #${current.id}`
  ).slice(0, 255);
  ...
  await pool.query(
    `UPDATE rooms
     ...
     WHERE id = ?`,
    [nextRoomStatus, notes, room.id],
  );

  const [updatedRoomRows] = await pool.query(
    `SELECT id, room_code, room_status, notes, last_status_updated_at
     FROM rooms
     WHERE id = ?
     LIMIT 1`,
    [room.id],
  ...
  const updatedBlock = await getRoomBlockRowById(id);
  ...
  await recordAuditLog({
    ...
    new_status: nextRoomStatus,
    action_by_user_id: user.id,
    action_notes: {
      room_block_request_id: current.id,
      room_code: room.room_code,
      notes,
    },
  });
  ...
  return {
    ok: true,
    block: formatRoomBlock(updatedBlock),
    room: updatedRoomRows[0] || null,
  };
}
...
async function reviewRoomBlock(id, input, user) {
  ...
  if (!BLOCK_REVIEW_STATUSES.has(nextStatus)) {
    return {
      ok: false,
      statusCode: 400,
      message: "block_status must be approved or rejected",
    };
  }
