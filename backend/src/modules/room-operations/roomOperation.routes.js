...
const roomIssueListValidator = [
  ...
  query("issue_type")
    .optional()
    .isIn(["computer", "network", "projector", "power", "other"])
...
const createRoomBlockValidator = [
  ...
  body("block_status")
    ...
    .withMessage("block_status must be draft or submitted"),
  body("block_title")
    .trim()
    .notEmpty()
...
const updateRoomBlockValidator = [...idValidator, ...createRoomBlockValidator];

const submitRoomBlockValidator = [
  ...idValidator,
  body("submit_notes")
    .optional({ nullable: true })
    .isString()
    .withMessage("submit_notes must be a string"),
];

const cancelRoomBlockValidator = [
  ...idValidator,
  body("review_notes")
    .optional({ nullable: true })
    .isString()
...
const applyRoomBlockStatusValidator = [
  ...idValidator,
  body("room_status")
    .isIn(["available", "maintenance", "out_of_order", "locked"])
    .withMessage("room_status is not supported"),
  body("notes")
    .optional({ nullable: true })
    .isString()
...
router.get(
  "/room-block-requests",
  requireRoles(ROLES.TECHNICIAN, ROLES.ACADEMIC_OFFICER),
...
router.patch(
  "/room-block-requests/:id",
  requireRoles(ROLES.TECHNICIAN, ROLES.ACADEMIC_OFFICER, ROLES.ADMIN),
  updateRoomBlockValidator,
  asyncHandler(updateRoomBlock),
);

router.patch(
  "/room-block-requests/:id/submit",
  requireRoles(ROLES.TECHNICIAN, ROLES.ACADEMIC_OFFICER, ROLES.ADMIN),
  submitRoomBlockValidator,
  asyncHandler(submitRoomBlock),
);

router.patch(
  "/room-block-requests/:id/cancel",
  requireRoles(ROLES.TECHNICIAN, ROLES.ACADEMIC_OFFICER, ROLES.ADMIN),
  cancelRoomBlockValidator,
  asyncHandler(cancelRoomBlock),
);
...
router.patch(
  "/room-block-requests/:id/apply-room-status",
  requireRoles(ROLES.ACADEMIC_OFFICER),
  applyRoomBlockStatusValidator,
  asyncHandler(applyRoomBlockStatus),
);
