...
function New-PortableDumpPath {
  param([string]$SourcePath)

  $utf8 = [System.Text.UTF8Encoding]::new($false, $true)
  $sql = [System.IO.File]::ReadAllText($SourcePath, $utf8)
  $sql = $sql -replace '/\*!50013 DEFINER=.*? SQL SECURITY DEFINER \*/\r?\n', ''

  $tempPath = Join-Path ([System.IO.Path]::GetTempPath()) ("lab-schedule-reset-dump-{0}.sql" -f $PID)
  [System.IO.File]::WriteAllText($tempPath, $sql, $utf8)
  return $tempPath
}

$script:ResolvedMysqlPath = Resolve-MysqlPath $MysqlPath
$repoRoot = Resolve-Path (Join-Path $PSScriptRoot "..")
$sourceDumpPath = (Resolve-Path (Join-Path $repoRoot "database\Dump20260428.sql")).Path
$portableDumpPath = New-PortableDumpPath $sourceDumpPath
$dumpPath = $portableDumpPath.Replace("\", "/")
...
"@
  ...
  Remove-Item -LiteralPath $portableDumpPath -ErrorAction SilentlyContinue
