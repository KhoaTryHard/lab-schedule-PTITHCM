...
export function buildScheduleWeeksQueryString(params = {}) {
  const query = new URLSearchParams();

  appendParam(query, "semester_id", params.semester_id);
  appendParam(query, "all", params.all);
 ...
 * Dùng cho màn lịch GV/SV/KTV để hiển thị đủ tuần học của học kỳ.
 */
export function listScheduleWeeks(params = {}) {
  return apiClient(`/schedules/weeks${buildScheduleWeeksQueryString(params)}`, {
    method: "GET",
  });
}
 ...
 * Dùng để lấy danh mục ca/tiết từ bảng time_slots.
 */
export function listTimeSlots() {
  return apiClient("/schedules/time-slots", {
    method: "GET",
  });
}
