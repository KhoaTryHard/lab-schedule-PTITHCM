...
function normalizeScheduleItem(item, index) {
  ...
  return {
    id,
    dataType: "schedule",
    schedule_code: getDisplayValue(
      item?.schedule_code ?? item?.id ?? item?.schedule_id,
    ),
    course_label:
...
function normalizeScheduleRequestItem(item, index) {
  ...
  return {
    ...
    request_code: getDisplayValue(
      item?.request_code ?? item?.id ?? item?.request_id,
    ),
    course_label:
      item?.course_code || item?.course_name
        ? `${item?.course_code || "—"} - ${item?.course_name || "—"}`
...
function getEmptyDescription(activeTab) {
  if (activeTab === "schedules") {
    return "Frontend đã gọi GET /api/schedules, nhưng hiện chưa có dữ liệu lịch phù hợp để hiển thị.";
  }
