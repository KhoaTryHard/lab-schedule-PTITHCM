  ...
  return {
    ...
    courseLabel:
      courseCode && courseName
        ? `${courseCode} - ${courseName}`
        : courseName || courseCode || `Lịch dạy #${index + 1}`,
  ...
  async function loadScheduleData(activeFilters = appliedFilters) {
    try {
      ...
      const response = await listSchedules({
        ...activeFilters,
        status: "published",
        lecturer_user_id: currentUser?.id,
      });
  ...
  return (
    <div className="adminPageStack">
      <section className="card">
        <div className="roomFilterBar">
          <div className="roomFilterSummary">
            <h1 className="roomSectionTitle">Lịch dạy thực hành của giảng viên</h1>
            <p className="roomSectionText">
              Gọi API thật GET /api/schedules với status=published và lecturer_user_id của tài khoản hiện tại.
            </p>
          </div>
        ...
        <form onSubmit={handleSubmit} style={FILTER_GRID_STYLE}>
          ...
          <label style={FIELD_STYLE}>
            ...
            <input
              ...
              placeholder="Học phần, lớp, phòng..."
            />
          </label>
      ...
      <section className="card">
        ...
        <p className="roomSectionText">
          Backend hiện chưa implement role scope cho GV. Frontend đã gửi query:
